...
static bool node_iskey(const void *val, const void *key, size_t len) {
  const SpRingNode *node = val;
  return node->keylen == len && memcmp(node->key, key, len) == 0;
}

static void node_free(void *val) {
  SpRingNode *node = val;
  sp_free(node, sizeof *node + node->keylen);
}

static const SpType map_type = {
    .hash = sp_siphash, .iskey = node_iskey, .free = node_free};

static uint64_t make_hash(const SpRing *self, const void *key, size_t len) {
  return self->hash(key, len, SP_SEED_DEFAULT) % 4294967291ULL;
}

int sp_ring_init(SpRing *self, SpHash fn) {
  assert(self != NULL);
  assert(fn != NULL);
...
void sp_ring_final(SpRing *self) {
  if (self != NULL) {
...
static SpRingNode *create_node(SpRing *self, const void *restrict key,
                               size_t len, int avail) {
...
static int cmp_replica(const void *a, const void *b) {
...
static void replicate(SpRing *self, SpRingNode *node, const void *restrict key,
                      size_t len, unsigned replicas) {
  char buf[1024], *p;
  ...
  for (unsigned i = 0; i < replicas; i++) {
    ...
    sp_vec_push(
        self->replicas,
        ((SpRingReplica){.hash = make_hash(self, buf, len + n), .node = node}));
  }

  sp_vec_sort(self->replicas, cmp_replica);
}

int sp_ring_put(SpRing *self, const void *restrict key, size_t len,
...
const SpRingNode *sp_ring_get(const SpRing *self, const void *restrict key,
                              size_t len) {
  return sp_map_get(&self->nodes, key, len);
}

bool sp_ring_del(SpRing *self, const void *restrict key, size_t len) {
  assert(self != NULL);
  assert(key != NULL);
...
const SpRingReplica *sp_ring_find(const SpRing *self, const void *restrict val,
                                  size_t len) {
  assert(self != NULL);
  assert(val != NULL);
...
const SpRingReplica *sp_ring_next(const SpRing *self,
...
const SpRingNode *sp_ring_reserve(const SpRing *self,
                                  const SpRingReplica *rep) {
  assert(self != NULL);
  assert(rep != NULL);
  assert(rep >= self->replicas &&
         rep < self->replicas + sp_vec_count(self->replicas));
...
void sp_ring_restore(const SpRing *self, const SpRingNode *node) {
  assert(self != NULL);
...
void sp_ring_print(const SpRing *self, FILE *out) {
  if (out == NULL) {
    out = stderr;
  }
  ...
  if (self == NULL) {
    ...
    fprintf(out, "#<SpRing:%p nodes=%zu, replicas=%zu> {\n", (void *)self,
            sp_map_count(&self->nodes), sp_vec_count(self->replicas));
