...
void sp_path_pop(const char *path, SpRange16 *rng, int n) {
  assert(path != NULL);
  assert(rng != NULL);
...
static void split_left(SpRange16 *a, SpRange16 *b, const char *path,
                       uint16_t plen, int n) {
  assert(plen > 0);
...
static void split_right(SpRange16 *a, SpRange16 *b, const char *path,
                        uint16_t plen, int n) {
  assert(plen > 0);
...
void sp_path_split(SpRange16 *a, SpRange16 *b, const char *path, uint16_t plen,
                   int n) {
  assert(a != NULL);
  assert(b != NULL);
  assert(path != NULL);
...
void sp_path_splitext(SpRange16 *a, SpRange16 *b, const char *path,
                      uint16_t plen) {
  assert(a != NULL);
  assert(b != NULL);
  assert(path != NULL);
...
static inline bool has_suffix_dir(const char *buf, uint16_t len) {
  return (len == 1 && buf[0] == '.') ||
         (len == 2 && buf[0] == '.' && buf[1] == '.') ||
         (len >= 2 && buf[len - 2] == '/' && buf[len - 1] == '.') ||
         (len >= 3 && buf[len - 3] == '/' && buf[len - 2] == '.' &&
          buf[len - 1] == '.');
}

int sp_path_join(char *out, size_t len, const char *a, uint16_t alen,
                 const char *b, uint16_t blen, SpPathMode mode) {
...
/*
 * Changes path to the shortest name equivalent to path by purely lexical
 ...
 */
uint16_t sp_path_clean(char *path, uint16_t len, SpPathMode mode) {
  assert(path != NULL);
...
static bool match_bracket(const char **p, const char **m) {
  bool neg = false;
  if (**m == '!' || **m == '^') {
    (*m)++;
    neg = true;
...
static bool match_brace(const char **p, const char **m) {
  const char *mark = *m;
  ssize_t len = -1;
  while (1) {
    switch (**m) {
...
bool sp_path_match(const char *path, const char *match) {
  assert(path != NULL);
  assert(match != NULL);
...
bool sp_path_suffix(const char *path, const char *match) {
  if (*match != '/') {
...
int sp_path_proc(char *buf, size_t buflen) {
  ssize_t len = -1;
...
int sp_path_env(const char *name, char *buf, size_t buflen) {
...
int sp_dir_open(SpDir *self, const char *path, uint8_t depth) {
...
static bool is_rel_dir(struct dirent *ent) {
  return ent->d_name[0] == '.' &&
         (ent->d_name[1] == '\0' ||
          (ent->d_name[1] == '.' && ent->d_name[2] == '\0'));
}

static int walk(SpDir *self) {
  struct dirent *ent;
  DIR *dir;
...
int sp_dir_next(SpDir *self) {
  ...
  if (self->cur < self->max && !(self->flags & F_SKIP) &&
...
void sp_dir_skip(SpDir *self) {
  assert(self != NULL);

  self->flags |= F_SKIP;
}

int sp_dir_follow(SpDir *self) {
  assert(self != NULL);
...
SpPathType sp_dir_type(SpDir *self) {
  assert(self != NULL);

  return IFTODT(self->stat.mode);
}

const SpStat *sp_dir_stat(SpDir *self) {
  assert(self != NULL);
...
void sp_dir_pathname(const SpDir *self, const char **start, size_t *len) {
  assert(self != NULL);
...
void sp_dir_dirname(const SpDir *self, const char **start, size_t *len) {
  assert(self != NULL);
...
void sp_dir_basename(const SpDir *self, const char **start, size_t *len) {
  assert(self != NULL);
...
static inline void copy_stat(SpStat *sbuf, const struct stat *s) {
...
int sp_stat(const char *path, SpStat *sbuf, bool follow) {
  struct stat s;
...
int sp_fstat(int fd, SpStat *sbuf) {
  struct stat s;
  int rc = fstat(fd, &s);
  if (rc < 0) {
    return SP_ESYSTEM(errno);
