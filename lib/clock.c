...
static void __attribute__((constructor)) init(void) {
  if (host_get_clock_service(mach_host_self(), CALENDAR_CLOCK, &clock_serv) !=
      KERN_SUCCESS) {
...
int sp_clock_real(SpClock *clock) {
  assert(clock != NULL);
...
int sp_clock_mono(SpClock *clock) {
  assert(clock != NULL);
...
double sp_clock_diff(SpClock *clock) {
  assert(clock != NULL);
...
double sp_clock_step(SpClock *clock) {
  assert(clock != NULL);
...
void sp_clock_print(const SpClock *clock, FILE *out) {
  if (out == NULL) {
    out = stderr;
  }
