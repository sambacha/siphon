...
ssize_t sp_fmt_str(FILE *out, const void *restrict str, size_t len,
                   bool quote) {
  ssize_t total = 0;
...
ssize_t sp_fmt_char(FILE *out, int c) {
  if (c < 0 || c > 255) {
    return 0;
  }
...
ssize_t sp_fmt_bytes(FILE *out, const void *in, size_t len, uintptr_t addr) {
