...
#define STACK_CS(p) (STACK_IN_OBJ(p) ? OBJECT_NEXT : ARRAY_NEXT)

#define YIELD_CS(p) (p->depth ? STACK_CS(p) : DONE)

#define YIELD_STACK(typ) YIELD(typ, YIELD_CS(p))

#define YIELD_IF_POP_ARR()                                                     \
  do {                                                                         \
...
static ssize_t parse_string(SpJson *restrict p, const uint8_t *restrict m,
                            size_t len, bool eof) {
...
again:
  EXPECT_RANGE(rng_check, SP_JSON_MAX_STRING, eof, SP_JSON_ESYNTAX,
               SP_JSON_ESIZE);
...
static ssize_t parse_number(SpJson *restrict p, const uint8_t *restrict m,
                            size_t len, bool eof) {
  // TODO: use better number parser

  static const uint8_t rng_non_num[] =
      "\x00\x2a\x2c\x2c\x2f\x2f\x3a\x44\x46\x64\x66\xff";
...
static size_t parse_true(SpJson *restrict p, const uint8_t *restrict m,
                         size_t len, bool eof) {
...
static size_t parse_false(SpJson *restrict p, const uint8_t *restrict m,
                          size_t len, bool eof) {
...
static size_t parse_null(SpJson *restrict p, const uint8_t *restrict m,
                         size_t len, bool eof) {
...
static ssize_t parse_any(SpJson *restrict p, const uint8_t *restrict m,
                         size_t len, bool eof) {
  const uint8_t *end = m + p->off;
...
static ssize_t parse_array(SpJson *restrict p, const uint8_t *restrict m,
                           size_t len, bool eof) {
  const uint8_t *end = m + p->off;
...
static ssize_t parse_object(SpJson *restrict p, const uint8_t *restrict m,
                            size_t len, bool eof) {
  const uint8_t *end = m + p->off;
...
static inline void init_values(SpJson *p) {
  p->number = 0.0;
  p->type = SP_JSON_NONE;
  p->cs = 0;
  p->off = 0;
...
ssize_t sp_json_next(SpJson *p, const void *restrict buf, size_t len,
                     bool eof) {
  assert(p != NULL);
  ...
  switch (p->cs) {
  case KIND_ANY:
    return parse_any(p, buf, len, eof);
  case KIND_KEY: /* fallthrough */
  case KIND_STRING:
    return parse_string(p, buf, len, eof);
  case KIND_NUMBER:
    return parse_number(p, buf, len, eof);
  case KIND_TRUE:
    return parse_true(p, buf, len, eof);
  case KIND_FALSE:
    return parse_false(p, buf, len, eof);
  case KIND_NULL:
    return parse_null(p, buf, len, eof);
  }
...
bool sp_json_in_object(const SpJson *p) {
  assert(p != NULL);

  return p->depth > 0 && STACK_IN_OBJ(p);
}

bool sp_json_is_key(const SpJson *p) {
  assert(p != NULL);

  return p->cs == OBJECT_SEP;
}

bool sp_json_in_array(const SpJson *p) {
  assert(p != NULL);

  return p->depth > 0 && STACK_IN_ARR(p);
}

uint8_t *sp_json_steal_string(SpJson *p, size_t *len, size_t *cap) {
  assert(p != NULL);
