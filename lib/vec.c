...
static inline SpVec *resize(SpVec *v, void **vec, size_t size, size_t cap,
                            size_t count) {
  size_t total = sizeof *v + size * cap;
  total = total < 16276 ? sp_power_of_2(total) : sp_next_quantum(total, 4096);
...
static inline SpVec *ensure(SpVec *v, void **vec, size_t size, size_t nitems) {
...
size_t sp_vecp_count(void **vec) { return VEC(vec)->count; }

size_t sp_vecp_capacity(void **vec) { return VEC(vec)->capacity; }

void sp_vecp_clear(void **vec) {
  SpVec *v = VEC(vec);
  v->count = 0;
}

void sp_vecp_free(void **vec, size_t size) {
  SpVec *v = VEC(vec);
  if (v != &NIL) {
...
int sp_vecp_ensure(void **vec, size_t size, size_t nitems) {
  if (nitems == 0) {
    return 0;
  }
...
int sp_vecp_push(void **vec, size_t size, const void *el, size_t nitems) {
  if (nitems == 0) {
    return 0;
  }
...
int sp_vecp_pop(void **vec, size_t size, void *el, size_t nitems) {
  if (nitems == 0) {
    return 0;
  }
...
int sp_vecp_shift(void **vec, size_t size, void *el, size_t nitems) {
  if (nitems == 0) {
    return 0;
  }
...
int sp_vecp_splice(void **vec, size_t size, ssize_t start, ssize_t end,
...
void sp_vecp_reverse(void **vec, size_t size, void *tmp) {
  SpVec *v = VEC(vec);
