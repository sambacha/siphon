...
ssize_t sp_uri_copy(SpUri *dst, char *dstbuf, size_t dstlen, const SpUri *u,
                    const char *ubuf) {
  assert(dst != NULL);
  assert(dstbuf != NULL);
  assert(u != NULL);
...
ssize_t sp_uri_join(SpUri *dst, char *dstbuf, size_t dstlen, const SpUri *a,
                    const char *abuf, const SpUri *b, const char *bbuf) {
  assert(dst != NULL);
  assert(dstbuf != NULL);
  ...
  if (seg == SP_URI_PATH) {
    ...
    int plen = sp_uri_join_paths(p, dstlen - (p - dstbuf), abuf + rng.off,
                                 rng.len, bbuf + b->seg[SP_URI_PATH].off,
                                 b->seg[SP_URI_PATH].len);
...
ssize_t sp_uri_join_paths(char *outbuf, size_t len, const char *abuf,
                          size_t alen, const char *bbuf, size_t blen) {
  assert(outbuf != NULL);
  assert(abuf != NULL);
  assert(bbuf != NULL);
  ...
  ssize_t plen =
      sp_path_join(outbuf, len, abuf, rng.len, bbuf, blen, SP_PATH_URI);
...
bool sp_uri_eq(const SpUri *a, const char *abuf, const SpUri *b,
               const char *bbuf) {
  assert(a != NULL);
  assert(abuf != NULL);
  assert(b != NULL);
  ...
  if (a == b)
    return true;
  if (a == NULL || b == NULL || a->first != b->first || a->last != b->last ||
...
uint16_t sp_uri_length(const SpUri *u) {
  assert(u != NULL);

  return u->seg[u->last].off + u->seg[u->last].len;
}

SpUriSegment sp_uri_sub(const SpUri *u, SpUriSegment start, SpUriSegment end,
                        SpRange16 *out) {
  assert(u != NULL);
  assert(out != NULL);
...
SpUriSegment sp_uri_range(const SpUri *u, SpUriSegment start, SpUriSegment end,
                          SpRange16 *out) {
  assert(u != NULL);
  assert(out != NULL);
...
int sp_uri_adjust_range(const SpUri *u, SpUriSegment *start, SpUriSegment *end,
                        bool valid) {
  assert(u != NULL);
...
SpUriSegment sp_uri_find_segment(const SpUri *u, SpUriSegment start,
                                 bool nonempty) {
  assert(u != NULL);
...
SpUriSegment sp_uri_rfind_segment(const SpUri *u, SpUriSegment start,
                                  bool nonempty) {
  assert(u != NULL);
...
bool sp_uri_has_segment(const SpUri *u, SpUriSegment seg) {
  assert(u != NULL);

  return (seg == SP_URI_PATH || (SP_URI_SEGMENT_VALID(seg) && u->seg[seg].len));
}

bool sp_uri_is_absolute(const SpUri *u) {
  return (u && sp_uri_has_segment(u, SP_URI_SCHEME) &&
          sp_uri_has_segment(u, SP_URI_HOST));
}

const char *sp_uri_segment_name(SpUriSegment seg) {
  static const char *names[] = {
      [SP_URI_SCHEME] = "scheme",     [SP_URI_USER] = "user",
      [SP_URI_PASSWORD] = "password", [SP_URI_HOST] = "host",
      [SP_URI_PORT] = "port",         [SP_URI_PATH] = "path",
      [SP_URI_QUERY] = "query",       [SP_URI_FRAGMENT] = "fragment"};
...
static void print_segment(const SpUri *u, const char *buf, FILE *out,
                          SpUriSegment seg) {
  if (!sp_uri_has_segment(u, seg)) {
    return;
  }
  fprintf(out, "    %s[%u, %u]", sp_uri_segment_name(seg), u->seg[seg].off,
          u->seg[seg].len);
  if (buf != NULL) {
    fprintf(out, " = \"%.*s\"", (int)u->seg[seg].len, buf + u->seg[seg].off);
...
void sp_uri_print(const SpUri *u, const char *buf, FILE *out) {
  if (out == NULL) {
    out = stderr;
  }
...
ssize_t sp_uri_query_next(const void *restrict buf, size_t len,
