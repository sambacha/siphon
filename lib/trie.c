...
static inline uint32_t clamp(uint32_t v) { return min(v, PREFIX_MAX); }

static inline size_t common_prefix(const uint8_t *k, size_t klen,
...
static inline size_t common_prefix_clamp(const uint8_t *k, size_t klen,
                                         const uint8_t *v, size_t vlen) {
...
static size_t leaf_prefix(const SpTrieLeaf *l, const uint8_t *k, size_t klen,
                          size_t off) {
  assert(off <= klen);
  ...
  return common_prefix(k + off, klen - off, l->key + off, l->key_len - off) +
         off;
}

static size_t leaf_prefix_clamp(const SpTrieLeaf *l, const uint8_t *k,
                                size_t klen, size_t off) {
  assert(off <= klen);

  if (off > l->key_len) {
    return off;
  }
  return common_prefix_clamp(k + off, klen - off, l->key + off,
                             l->key_len - off) +
         off;
}

static size_t branch_prefix_clamp(const SpTrieBranch *b, const uint8_t *k,
                                  size_t klen, size_t off) {
  assert(off <= klen);

  return common_prefix_clamp(k + off, klen - off, b->prefix, b->prefix_len) +
         off;
}

int sp_trie_init(SpTrie *self, const SpType *type) {
  assert(self != NULL);
...
void sp_trie_final(SpTrie *self) {
  assert(self != NULL);
...
size_t sp_trie_count(const SpTrie *self) {
  assert(self != NULL);

  return self->count;
}

static void free_leaf(SpTrieLeaf *l) {
  assert(IS_LEAF((SpTrieNode *)l));
  sp_free(l, sizeof *l + l->key_len);
}

static void free_branch(SpTrieBranch *b) {
  assert(IS_BRANCH((SpTrieNode *)b));
  sp_free(b, sizeof *b + CAPACITY(b) * sizeof b->children[0]);
}

static void clear(SpTrieNode *n, SpFree func) {
  if (n == NULL) {
    return;
  }
...
void sp_trie_clear(SpTrie *self) {
  assert(self != NULL);
...
static SpTrieLeaf **get(SpTrieNode **par, const void *restrict key, size_t len,
...
static SpTrieLeaf *match(const SpTrie *self, const void *restrict key,
                         size_t len, size_t *off, SpTrieMatch cb, void *data) {
  ...
  while (n && offset <= len) {
    if (IS_LEAF(n)) {
      ...
      if (b->leaf && (end - offset) == b->prefix_len &&
...
static bool cmp(const void *key, size_t len, size_t off, void *val,
                void *data) {
  (void)val;
  int c = *(int *)data;
...
static SpTrieLeaf *prefix(const SpTrie *self, const void *restrict key,
                          size_t len, size_t *off, int c) {
  return match(self, key, len, off, cmp, &c);
}

bool sp_trie_has_key(const SpTrie *self, const void *restrict key, size_t len) {
  assert(self != NULL);
  assert(key != NULL);
...
bool sp_trie_has_prefix(const SpTrie *self, const void *restrict key,
                        size_t len, size_t *off, int c) {
  assert(self != NULL);
  assert(key != NULL);
...
bool sp_trie_has_match(const SpTrie *self, const void *restrict key, size_t len,
...
void *sp_trie_get(const SpTrie *self, const void *restrict key, size_t len) {
  assert(self != NULL);
  assert(key != NULL);
...
void *sp_trie_prefix(const SpTrie *self, const void *restrict key, size_t len,
                     size_t *off, int c) {
  assert(self != NULL);
  assert(key != NULL);
...
void *sp_trie_match(const SpTrie *self, const void *restrict key, size_t len,
                    SpTrieMatch cb, void *data) {
  assert(self != NULL);
  assert(key != NULL);
...
int sp_trie_put(SpTrie *self, const void *restrict key, size_t len, void *val) {
  assert(self != NULL);
  assert(key != NULL);
 ...
 */
static SpTrieNode **reserve(SpTrieBranch **ref, uint8_t c) {
  assert(ref != NULL);
  assert(*ref != NULL);
  ...
  if (low || hi) {
    ...
    b = sp_realloc(b, sizeof *b + CAPACITY(b) * sizeof b->children[0],
...
static SpTrieBranch *branch_create(uint8_t c1, uint8_t c2, const void *key,
                                   size_t len) {
...
void **sp_trie_reserve(SpTrie *self, const void *restrict key, size_t len,
                       bool *isnew) {
  assert(self != NULL);
  assert(key != NULL);
  assert(isnew != NULL);
  ...
  while (*par != NULL) {
    ...
    if (IS_LEAF(*par)) {
      ...
      SpTrieBranch *b =
          branch_create(c1, c2, (uint8_t *)key + offset, end - offset);
      if (b == NULL)
        return NULL;
...
static bool compact(SpTrieBranch **b) {
  ...
  if (child != NULL) {
    // if child is a branch then join the two key prefixes plus the key
    // character
    if (IS_BRANCH(*child)) {
...
bool sp_trie_del(SpTrie *self, const void *restrict key, size_t len) {
  assert(self != NULL);
  assert(key != NULL);
...
void *sp_trie_steal(SpTrie *self, const void *restrict key, size_t len) {
  assert(self != NULL);
  assert(key != NULL);
...
static bool each(const SpTrieNode *n, SpTrieCallback func, void *data) {
  if (n == NULL) {
    return true;
  }
  if (IS_LEAF(n)) {
...
bool sp_trie_each(const SpTrie *self, SpTrieCallback func, void *data) {
  assert(self != NULL);
  assert(func != NULL);
...
bool sp_trie_each_prefix(const SpTrie *self, const void *restrict key,
                         size_t len, SpTrieCallback func, void *data) {
  assert(self != NULL);
  assert(key != NULL);
  assert(func != NULL);
...
static void print_leaf(const SpTrieLeaf *l, FILE *out, SpPrint print) {
...
static void print_branch(const SpTrieBranch *b, FILE *out, SpPrint print) {
...
static void print_list(const SpTrieNode *n, FILE *out, int c, bool *stack,
                       int top, SpPrint print) {
  if (n == NULL)
    return;
...
void sp_trie_print(const SpTrie *self, FILE *out) {
  if (out == NULL) {
    out = stderr;
  }
