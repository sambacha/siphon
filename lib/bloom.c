...
static inline void bloom_type(size_t hint, double fpp, uint64_t *bits,
                              uint64_t *bytes, uint8_t *hashes) {
...
uint64_t sp_bloom_hash(const void *restrict buf, size_t len) {
  assert(buf != NULL);

  return sp_metrohash64(buf, len, SP_SEED_DEFAULT);
}

SpBloom *sp_bloom_new(size_t hint, double fpp) {
  uint64_t bits;
  uint64_t bytes;
  uint8_t hashes;
...
void sp_bloom_free(SpBloom *self) {
  if (self != NULL) {
...
void sp_bloom_print(const SpBloom *self, FILE *out) {
  if (out == NULL) {
    out = stderr;
  }
  ...
  if (self == NULL) {
    ...
    fprintf(out,
            "#<SpBloom:%p fpp=%f, count=%" PRIu64 ", bits=%" PRIu64
            ", hashes=%u>\n",
