...
static ssize_t parse_meta(SpWs *restrict p, const uint8_t *const restrict m,
                          const size_t len) {
...
static ssize_t parse_paylen(SpWs *restrict p, const uint8_t *const restrict m,
                            const size_t len) {
  const uint8_t *end = m;
...
static ssize_t parse_mask_key(SpWs *restrict p, const uint8_t *const restrict m,
                              const size_t len) {
  const uint8_t *end = m;
...
static ssize_t parse_payload(SpWs *restrict p, const uint8_t *const restrict m,
                             const size_t len) {
  const uint8_t *end = m;
  uint64_t plen = 0;
...
static size_t mask(void *dst, const void *restrict src, size_t len,
                   const uint8_t *key, size_t off) {
  /**
...
void sp_ws_reset(SpWs *p) {
  assert(p != NULL);

  sp_ws_init(p);
}

ssize_t sp_ws_next(SpWs *p, const void *restrict buf, size_t len) {
  assert(p != NULL);
  ...
  if (p->cs & META)
    rc = parse_meta(p, buf, len);
  else if (p->cs & LEN)
    rc = parse_paylen(p, buf, len);
  else if (p->cs & MASK)
    rc = parse_mask_key(p, buf, len);
  else if (p->cs & PAYLOAD)
    rc = parse_payload(p, buf, len);
  else {
    YIELD_ERROR(SP_WS_ESTATE);
  }
  if (rc > 0) {
...
bool sp_ws_is_done(const SpWs *p) {
  assert(p != NULL);

  return IS_DONE(p->cs);
}

size_t sp_ws_unmask(SpWs *p, void *dst, const void *restrict buf, size_t len) {
  if (p->type < SP_WS_PAYLOAD)
    return SP_WS_ESTATE;

  if (len > p->off)
    return SP_WS_EUMASKMAX;

  return mask(dst, buf, len, p->as.mask_key, p->off - len);
}

size_t sp_ws_mask(void *dst, const void *restrict src, size_t len,
                  const uint8_t *key) {
  return mask(dst, src, len, key, 0);
}

ssize_t sp_ws_enc_frame(void *buf, const SpWsFrame *f) {
  uint8_t *end = buf;
  uint8_t meta = 0;
  ...
  *(uint8_t *)end = meta;
  ...
  meta = 0;
  if (f->paylen.type == SP_WS_LEN_7)
    meta = f->paylen.len.u7 & PAYLEN_MASK;
  if (f->paylen.type == SP_WS_LEN_16)
    meta = LEN_16_CODE & PAYLEN_MASK;
  if (f->paylen.type == SP_WS_LEN_64)
    meta = LEN_64_CODE & PAYLEN_MASK;
  if (f->masked)
    meta = MASK_MASK | meta;
  *(uint8_t *)end = meta;
  end++;
  ...
  if (f->paylen.type == SP_WS_LEN_64) {
    if (f->paylen.len.u64 > LEN_MAX)
      return SP_WS_ELENMAX;
...
ssize_t sp_ws_enc_ctrl(void *buf, const SpWsCtrlOpcode code, size_t len,
                       const uint8_t *key) {
  SpWsFrame f;
  memset(&f, 0, sizeof f);
...
ssize_t sp_ws_enc_ping(void *buf, size_t len, const uint8_t *key) {
  return sp_ws_enc_ctrl(buf, SP_WS_PING, len, key);
}

ssize_t sp_ws_enc_pong(void *buf, size_t len, const uint8_t *key) {
  return sp_ws_enc_ctrl(buf, SP_WS_PONG, len, key);
}

ssize_t sp_ws_enc_close(void *buf, SpWsStatus stat, size_t len,
                        const uint8_t *key) {
  uint8_t *end = buf;
  uint8_t *meta = buf;
  ...
  *(uint16_t *)end = sp_htobe16(stat);
  ...
  if (key)
    sp_ws_mask(meta, end - slen, slen, key);

  return end - (uint8_t *)buf;
}

ssize_t sp_ws_meta_length(const SpWs *p) {
  assert(p != NULL);

  if (p->type < SP_WS_META)
    return SP_WS_ESTATE;
...
ssize_t sp_ws_payload_length(const SpWs *p, uint64_t *len) {
  assert(p != NULL);
  ...
  case SP_WS_LEN_7:
    ...
    break;
  case SP_WS_LEN_16:
    (*len) = (uint64_t)p->as.paylen.len.u16;
    break;
  case SP_WS_LEN_64:
    (*len) = (uint64_t)p->as.paylen.len.u64;
    break;
  default:
    (*len) = 0;
  }
...
static char *bool_string(bool v) { return v ? "true" : "false"; }

static void print_paylen(const SpWs *p, FILE *out) {
  assert(p != NULL);

  SpWsLenType type = p->as.paylen.type;
  if (type == SP_WS_LEN_NONE || type == SP_WS_LEN_7)
    return;
...
static void print_meta(const SpWs *p, FILE *out) {
  assert(p != NULL);
...
static void print_mask_key(const SpWs *p, FILE *out) {
  assert(p != NULL);

  if (p->as.masked) {
    fprintf(out, "    mask key = [");
    for (size_t i = 0; i < MASK_KEY_BYTES; i++)
      fprintf(out, "0x%x%s", p->as.mask_key[i],
              (i < MASK_KEY_BYTES - 1 ? ", " : ""));
    fprintf(out, "]\n");
  }
}

void sp_ws_print_meta(const SpWs *p, const void *restrict buf, FILE *out) {
  if (p->type == SP_WS_PAYLOAD)
    return;
  ...
  if (p->type == SP_WS_META)
    len = META_BYTES;
  if (p->type == SP_WS_PAYLEN)
    len = sp_ws_meta_length(p) - META_BYTES;
  if (p->type == SP_WS_MASK_KEY)
    len = MASK_KEY_BYTES;
  for (i = 0; i < len; i++)
...
const char *sp_ws_status_string(SpWsStatus stat) {
  /**
   * Text as added to the IANA registry.
   * https://tools.ietf.org/html/rfc6455#section-11.7
   */
  switch (stat) {
  case SP_WS_STATUS_NORMAL:
    return "Normal Closure";
  case SP_WS_STATUS_AWAY:
    return "Going Away";
  case SP_WS_STATUS_PROTO:
    return "Protocol error";
  case SP_WS_STATUS_TYPE:
    return "Unsupported Data";
  case SP_WS_STATUS_DATA:
    return "Invalid frame payload data";
  case SP_WS_STATUS_POLICY:
    return "Policy Violation";
  case SP_WS_STATUS_BIG:
    return "Message Too Big";
  case SP_WS_STATUS_EXT:
    return "Mandatory Ext.";
  case SP_WS_STATUS_FAIL:
