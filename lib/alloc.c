...
static void __attribute__((constructor(110))) init(void) {
...
static size_t dbg_size(size_t size) {
  return ((((size + DBG_PREAMBLE_SIZE) - 1) / pagesize) + 1) * pagesize;
}

static struct dbg_preamble *dbg_get_preamble(void *p) {
  if (p == NULL) {
    return NULL;
  }
  return (struct dbg_preamble *)((char *)p -
                                 (DBG_PREAMBLE_SIZE +
                                  ((uintptr_t)p % DBG_PREAMBLE_SIZE)));
}

#define dbg_ensure_magic(p)                                                    \
  do {                                                                         \
...
static int dbg_protect(struct dbg_preamble *pre, int flags) {
  if (pre == NULL) {
    return 0;
  };
  dbg_ensure_magic(pre);
...
static void dbg_free(struct dbg_preamble *pre) {
...
void *sp_alloc_debug(void *ptr, size_t oldsz, size_t newsz, bool zero) {
  if (sp_unlikely(pagesize == 0)
#if SP_USE_VALGRIND
      || RUNNING_ON_VALGRIND
#endif
  ) {
  ...
  if (newsz > 0) {
    ...
    char *p = mmap(NULL, alloc + pagesize, PROT_READ | PROT_WRITE,
...
bool sp_alloc_summary(void) {
  ...
  sp_list_each(&leaks, entry, SP_ASCENDING) {
    struct dbg_preamble *pre =
        sp_container_of(entry, struct dbg_preamble, entry);
    fprintf(stderr, "*** leak of size %" PRIuPTR "\n%s", pre->size, pre->stack);
    leak += pre->size;
  }
