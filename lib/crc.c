...
#define NEXT4(t)                                                               \
  do {                                                                         \
    ...
    crc = t[0][c & 0xff] ^ t[1][(c >> 8) & 0xff] ^ t[2][(c >> 16) & 0xff] ^    \
          t[3][c >> 24];                                                       \
  } while (0)
...
uint32_t sp_crc32(uint32_t crc, const void *bytes, size_t len) {
  CRC32(table32);
  return crc;
}
...
static inline uint32_t crc32c_1024(uint32_t crc, const void *bytes) {
...
uint32_t sp_crc32c(uint32_t crc, const void *bytes, size_t len) {
  crc = ~crc;
