...
uint32_t sp_mix_uint32(uint32_t x) {
  x ^= x >> 16;
  x *= 0x85ebca6b;
  x ^= x >> 13;
  x *= 0xc2b2ae35;
...
uint32_t sp_mix_uint32s(uint32_t x, uint32_t y) {
...
uint64_t sp_mix_uint64(uint64_t x) {
  x ^= x >> 33;
...
uint64_t sp_mix_uint64s(uint64_t x, uint64_t y) {
...
inline static uint64_t rotr(uint64_t v, unsigned k) {
  return (v >> k) | (v << (64 - k));
}

inline static uint64_t rotl(uint64_t v, unsigned k) {
  return (v << k) | (v >> (64 - k));
}
...
uint64_t sp_metrohash64(const void *restrict s, size_t len,
                        const SpSeed *restrict seed) {
...
uint64_t sp_siphash(const void *restrict s, size_t len,
                    const SpSeed *restrict seed) {
  ...
  switch (len & 7) {
  case 7:
    b |= ((uint64_t)((uint8_t *)s)[6]) << 48;
  case 6:
    b |= ((uint64_t)((uint8_t *)s)[5]) << 40;
  case 5:
    b |= ((uint64_t)((uint8_t *)s)[4]) << 32;
  case 4:
    b |= ((uint64_t)((uint8_t *)s)[3]) << 24;
  case 3:
    b |= ((uint64_t)((uint8_t *)s)[2]) << 16;
  case 2:
    b |= ((uint64_t)((uint8_t *)s)[1]) << 8;
  case 1:
    b |= ((uint64_t)((uint8_t *)s)[0]);
    break;
  case 0:
    break;
  }
...
uint64_t sp_siphash_case(const void *restrict s, size_t len,
                         const SpSeed *restrict seed) {
  uint64_t b = ((uint64_t)len) << 56;
  uint64_t k0 = sp_le64toh(seed->u128.low);
  uint64_t k1 = sp_le64toh(seed->u128.high);
  ...
  switch (len & 7) {
  case 7:
    b |= ((uint64_t)lower[((uint8_t *)s)[6]]) << 48;
  case 6:
    b |= ((uint64_t)lower[((uint8_t *)s)[5]]) << 40;
  case 5:
    b |= ((uint64_t)lower[((uint8_t *)s)[4]]) << 32;
  case 4:
    b |= ((uint64_t)lower[((uint8_t *)s)[3]]) << 24;
  case 3:
    b |= ((uint64_t)lower[((uint8_t *)s)[2]]) << 16;
  case 2:
    b |= ((uint64_t)lower[((uint8_t *)s)[1]]) << 8;
  case 1:
    b |= ((uint64_t)lower[((uint8_t *)s)[0]]);
    break;
  case 0:
    break;
  }
...
uint64_t sp_xxhash64(const void *restrict input, size_t len,
                     const SpSeed *restrict seed) {
