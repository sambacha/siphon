...
#define B_IS_FINT(n)                                                           \
  (((n)&0xe0) == 0xe0)                  /* test if the tag is a fixed integer */
...
#define B_IS_FUINT(n)                                                          \
  (((n) >> 7) == 0)               /* test if the tag is a fixed unsigned */
#define B_FUINT(n) ((uint8_t)(n)) /* convert fixed unsigned into tag */
#define B_FUINT_VAL(n)                                                         \
  ((uint8_t)(n)) /* convert fixed unsigned tag into value */

#define B_FSTR_MAX 31 /* longest fixed string */
#define B_IS_FSTR(n)                                                           \
  (((n)&0xe0) == 0xa0)                  /* test if the tag is a fixed string */
#define B_FSTR(n) ((uint8_t)(n) | 0xa0) /* convert fixed string into tag */
#define B_FSTR_LEN(n)                                                          \
  ((n)&B_FSTR_MAX) /* convert fixed string tag into length */
...
#define YIELD_ERROR(err)                                                       \
  do {                                                                         \
    p->cs = DONE;                                                              \
    return err;                                                                \
  } while (0)

#define EXPECT_SIZE(sz, remain, done)                                          \
  do {                                                                         \
    if ((sz) > remain) {                                                       \
      if (done) {                                                              \
...
#define VERIFY_LEN(off, exp, remain, done)                                     \
  do {                                                                         \
 ...
 */
#define LOAD_UINT(p, bits, src, len, eof)                                      \
  do {                                                                         \
    EXPECT_SIZE(bits / 8 + 1, len, eof);                                       \
    p->type = SP_MSGPACK_UNSIGNED;                                             \
    p->tag.u64 =                                                               \
        (uint##bits##_t)sp_be##bits##toh(*(uint##bits##_t *)((src) + 1));      \
    return bits / 8 + 1;                                                       \
  } while (0)
 ...
 * @eof: if the byte length mark the end of input
 */
#define LOAD_SINT(p, bits, src, len, eof)                                      \
  do {                                                                         \
    EXPECT_SIZE(bits / 8 + 1, len, eof);                                       \
    int64_t val =                                                              \
        (int##bits##_t)sp_be##bits##toh(*(int##bits##_t *)((src) + 1));        \
    if (val < 0) {                                                             \
 ...
 */
#define WRITE_INT(buf, tag, T, val)                                            \
  do {                                                                         \
    *(uint8_t *)buf = (tag);                                                   \
...
#define PUSH_COUNT(p, n)                                                       \
  do {                                                                         \
    p->counts[p->depth - 1] = (n);                                             \
  } while (0)
...
void sp_msgpack_init(SpMsgpack *p) {
  memset(p, 0, sizeof *p);
  p->type = SP_MSGPACK_NONE;
}

void sp_msgpack_reset(SpMsgpack *p) { sp_msgpack_init(p); }

static inline ssize_t next(SpMsgpack *p, const uint8_t *restrict buf,
                           size_t len, bool eof) {
  ...
  case B_UINT8:
    ...
    return 2;
  case B_UINT16:
    LOAD_UINT(p, 16, buf, len, eof);
  case B_UINT32:
    LOAD_UINT(p, 32, buf, len, eof);
  case B_UINT64:
    LOAD_UINT(p, 64, buf, len, eof);
  ...
  case B_INT8:
    ...
    return 2;
  case B_INT16:
    LOAD_SINT(p, 16, buf, len, eof);
  case B_INT32:
    LOAD_SINT(p, 32, buf, len, eof);
  case B_INT64:
    LOAD_SINT(p, 64, buf, len, eof);
...
ssize_t sp_msgpack_next(SpMsgpack *p, const void *restrict buf, size_t len,
                        bool eof) {
  assert(p != NULL);
  assert(buf != NULL);
...
bool sp_msgpack_in_map(const SpMsgpack *p) {
  assert(p != NULL);

  return p->depth > 0 && STACK_IN_MAP(p);
}

bool sp_msgpack_is_key(const SpMsgpack *p) {
  assert(p != NULL);

  return sp_msgpack_in_map(p) && p->counts[p->depth - 1] % 2 == 1;
}

bool sp_msgpack_in_array(const SpMsgpack *p) {
  assert(p != NULL);

  return p->depth > 0 && STACK_IN_ARR(p);
}

bool sp_msgpack_is_done(const SpMsgpack *p) {
  assert(p != NULL);

  return IS_DONE(p->cs);
}

size_t sp_msgpack_enc(SpMsgpackType type, const SpMsgpackTag *tag, void *buf) {
  switch (type) {
  case SP_MSGPACK_NIL:
...
size_t sp_msgpack_enc_nil(void *buf) {
  *(uint8_t *)buf = B_NIL;
  return 1;
}

size_t sp_msgpack_enc_false(void *buf) {
  *(uint8_t *)buf = B_FALSE;
  return 1;
}

size_t sp_msgpack_enc_true(void *buf) {
  *(uint8_t *)buf = B_TRUE;
  return 1;
}

size_t sp_msgpack_enc_signed(void *buf, int64_t val) {
  if (val >= 0) {
...
size_t sp_msgpack_enc_unsigned(void *buf, uint64_t val) {
  if (val <= B_FUINT_MAX) {
...
size_t sp_msgpack_enc_float(void *buf, float val) {
  uint8_t *p = buf;
  *p = B_FLOAT;
...
size_t sp_msgpack_enc_double(void *buf, double val) {
  uint8_t *p = buf;
  *p = B_DOUBLE;
...
size_t sp_msgpack_enc_string(void *buf, uint32_t len) {
  if (len <= B_FSTR_MAX) {
...
size_t sp_msgpack_enc_binary(void *buf, uint32_t len) {
  if (len <= UINT8_MAX) {
...
size_t sp_msgpack_enc_array(void *buf, uint32_t count) {
  if (count <= B_FARR_MAX) {
...
size_t sp_msgpack_enc_map(void *buf, uint32_t count) {
  if (count <= B_FMAP_MAX) {
...
size_t sp_msgpack_enc_ext(void *buf, int8_t type, uint32_t len) {
  if (type < 0) {
    return 0;
  }
  ...
  if (len == 1) {
    ...
    *(uint8_t *)buf = B_FEXT16;
  } else if (len == 4) {
    *(uint8_t *)buf = B_FEXT32;
  } else if (len == 8) {
    *(uint8_t *)buf = B_FEXT64;
  } else if (len == 16) {
    *(uint8_t *)buf = B_FEXT128;
