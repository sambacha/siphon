...
static int cmp_code(int code, const SpError **val) {
  return (*val)->code - code;
}

static int cmp_err(const void *a, const void *b) {
  return cmp_code((*(const SpError **)a)->code, (const SpError **)b);
}

static inline void sort_errors(void) { sp_vec_sort(errors, cmp_err); }

static const SpError *create_error(int code, const char *domain,
...
static const SpError *get_error(int code) {
  const SpError **pos = sp_vec_bsearch(errors, code, cmp_code);
  return pos ? *pos : NULL;
}

static const SpError *push_error(int code, const char *domain, const char *name,
                                 const char *msg) {
  FIX_CODE(code);
...
#define COUNT(sym, msg) +1
  sp_vec_ensure(errors,
                2 * (0 SP_SYSTEM_ERRORS(COUNT) SP_EAI_ERRORS(COUNT)
                         SP_UTF8_ERRORS(COUNT) SP_HTTP_ERRORS(COUNT)
                             SP_JSON_ERRORS(COUNT) SP_MSGPACK_ERRORS(COUNT)
                                 SP_PATH_ERRORS(COUNT) SP_URI_ERRORS(COUNT)
                                     SP_WS_ERRORS(COUNT)));
#undef COUNT

#define PUSH_SYS(sym, msg) push_error(sym, "system", #sym, msg);
#define PUSH_EAI(sym, msg)                                                     \
  push_error(SP_EAI_CODE(EAI_##sym), "addr", "E" #sym, msg);
...
#define PUSH_MSGPACK(sym, msg)                                                 \
  push_error(SP_MSGPACK_##sym, "msgpack", #sym, msg);
...
int sp_eai_code(int err) { return SP_EAI_CODE(err); }

size_t sp_error_string(int code, char *buf, size_t size) {
  const SpError *err = sp_error(code);
  int len = 0;
  if (err == NULL) {
...
void sp_error_print(int code, FILE *out) {
  char buf[128];
  ssize_t len;
...
void sp_exit(int code, int exitcode) {
  sp_error_print(code, stderr);
  exit(exitcode);
}

static inline void stack_abort(char *buf, size_t len, size_t off) {
  if (off < len - 1) {
    buf[off++] = '\n';
...
const SpError *sp_error(int code) {
  FIX_CODE(code);
...
const SpError *sp_error_next(const SpError *err) {
  if (errors == NULL) {
    return NULL;
  }
...
const SpError *sp_error_add(int code, const char *domain, const char *name,
                            const char *msg) {
  FIX_CODE(code);
...
const SpError *sp_error_checkset(int code, const char *domain, const char *name,
                                 const char *msg) {
  FIX_CODE(code);
...
inline size_t sp_stack(char *buf, size_t len) {
#if SP_HAVE_EXECINFO
  char **strs = NULL;
  void *stack[32];
  int count = 0, i = 0;
