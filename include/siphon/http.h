...
SP_EXPORT int sp_http_init_request(SpHttp *p, bool capture);

SP_EXPORT int sp_http_init_response(SpHttp *p, bool capture);

SP_EXPORT void sp_http_final(SpHttp *p);

SP_EXPORT void sp_http_reset(SpHttp *p);

SP_EXPORT ssize_t sp_http_next(SpHttp *p, const void *restrict buf, size_t len);

SP_EXPORT bool sp_http_is_done(const SpHttp *p);

SP_EXPORT SpHttpMap *sp_http_steal_headers(SpHttp *p);

SP_EXPORT void sp_http_print(const SpHttp *p, const void *restrict buf,
                             FILE *out);

SP_EXPORT SpHttpMap *sp_http_map_new(void);

SP_EXPORT void sp_http_map_free(SpHttpMap *m);

SP_EXPORT int sp_http_map_put(SpHttpMap *m, const void *name, size_t nlen,
                              const void *value, size_t vlen);

SP_EXPORT bool sp_http_map_del(SpHttpMap *m, const void *name, size_t nlen);

SP_EXPORT void sp_http_map_clear(SpHttpMap *m);

SP_EXPORT const SpHttpEntry *sp_http_map_get(const SpHttpMap *m,
                                             const void *name, size_t nlen);

SP_EXPORT size_t sp_http_map_encode_size(const SpHttpMap *m);

SP_EXPORT void sp_http_map_encode(const SpHttpMap *m, void *buf);

SP_EXPORT size_t sp_http_map_scatter_count(const SpHttpMap *m);

SP_EXPORT void sp_http_map_scatter(const SpHttpMap *m, struct iovec *iov);

SP_EXPORT void sp_http_entry_name(const SpHttpEntry *e, struct iovec *iov);

SP_EXPORT size_t sp_http_entry_count(const SpHttpEntry *e);

SP_EXPORT bool sp_http_entry_value(const SpHttpEntry *e, size_t idx,
                                   struct iovec *iov);

SP_EXPORT void sp_http_map_print(const SpHttpMap *m, FILE *out);

SP_EXPORT ssize_t sp_cache_control_parse(SpCacheControl *cc, const char *buf,
                                         size_t len);
