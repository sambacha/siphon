...
#define SP_CLOCK_TIME(c) ((double)(c)->tv_sec + 1e-9 * (c)->tv_nsec)

#define SP_CLOCK_SET_TIME(c, time)                                             \
  do {                                                                         \
    double sec;                                                                \
...
#define SP_CLOCK_CMP(c, v, op)                                                 \
  ((c)->tv_sec == (v)->tv_sec ? (c)->tv_nsec op(v)->tv_nsec                    \
                              : (c)->tv_sec op(v)->tv_sec)

SP_EXPORT int sp_clock_real(SpClock *clock);

SP_EXPORT int sp_clock_mono(SpClock *clock);

SP_EXPORT double sp_clock_diff(SpClock *clock);

SP_EXPORT double sp_clock_step(SpClock *clock);

SP_EXPORT void sp_clock_print(const SpClock *clock, FILE *out);
