...
SP_EXPORT const char *sp_strerror(int code);

SP_EXPORT int sp_eai_code(int err);

SP_EXPORT size_t sp_error_string(int code, char *buf, size_t size);

SP_EXPORT void sp_error_print(int code, FILE *out);

SP_EXPORT void sp_exit(int code, int exitcode);

SP_EXPORT void sp_abort(int code);

SP_EXPORT void __attribute__((__format__(__printf__, 1, 2)))
sp_fabort(const char *fmt, ...);

SP_EXPORT const SpError *sp_error(int code);

SP_EXPORT const SpError *sp_error_next(const SpError *err);

SP_EXPORT const SpError *sp_error_add(int code, const char *domain,
                                      const char *name, const char *msg);

SP_EXPORT const SpError *sp_error_checkset(int code, const char *domain,
                                           const char *name, const char *msg);

SP_EXPORT size_t sp_stack(char *buf, size_t len);
