...
uint64_t junk_hash(const char *key) { return (uint64_t)*key + 1; }

bool junk_has_key(const char **junk, const char *key, size_t kn) {
  (void)kn;
...
#define TEST_ADD_NEW(map, key, resultcount)                                    \
  do {                                                                         \
    const char *k = key;                                                       \
...
#define TEST_ADD_OLD(map, key, resultcount)                                    \
  do {                                                                         \
    const char *k = key;                                                       \
    int rc = junk_put(map, k, &k);                                             \
...
#define TEST_REM_NEW(map, key, resultcount)                                    \
  do {                                                                         \
    const char *old = NULL;                                                    \
...
#define TEST_REM_OLD(map, key, resultcount)                                    \
  do {                                                                         \
    const char *old = NULL;                                                    \
    bool rc = junk_del(map, key, &old);                                        \
...
bool thing_has_key(Thing *t, int key, size_t len) {
  (void)len;
  return t->key == key;
}
...
uint64_t prehash_hash(uint64_t key, size_t len) {
  (void)len;
...
bool prehash_has_key(int *v, uint64_t key, size_t len) {
  (void)v;
  (void)key;
  (void)len;
