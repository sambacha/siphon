...
#define TEST_JOIN(base, rel, exp)                                              \
  do {                                                                         \
    char buf[1024];                                                            \
    int rc = sp_path_join(buf, sizeof buf, base, (sizeof base) - 1, rel,       \
                          (sizeof rel) - 1, 0);                                \
    mu_assert_int_gt(rc, 0);                                                   \
    mu_assert_str_eq(buf, exp);                                                \
  } while (0)

#define TEST_CLEAN(path, exp)                                                  \
  do {                                                                         \
    char buf[4096] = {0};                                                      \
...
#define TEST_CLEAN_URI(path, exp)                                              \
  do {                                                                         \
    char buf[4096] = {0};                                                      \
    memcpy(buf, path, sizeof path);                                            \
...
#define TEST_SPLIT(path, cnt, a, b)                                            \
  do {                                                                         \
    SpRange16 ra, rb;                                                          \
    char buf[4096];                                                            \
...
#define TEST_MATCH_VALID(a, b)                                                 \
  do {                                                                         \
    mu_assert_msg(sp_path_match(a, b), "Assertion '%s == %s' failed", a, b);   \
  } while (0)

#define TEST_MATCH_INVALID(a, b)                                               \
  do {                                                                         \
    mu_assert_msg(!sp_path_match(a, b), "Assertion '%s != %s' failed", a, b);  \
  } while (0)

#define TEST_SUFFIX_VALID(a, b)                                                \
  do {                                                                         \
    mu_assert_msg(sp_path_suffix(a, b), "Assertion '%s ends %s' failed", a,    \
                  b);                                                          \
  } while (0)

#define TEST_SUFFIX_INVALID(a, b)                                              \
  do {                                                                         \
    mu_assert_msg(!sp_path_suffix(a, b), "Assertion '%s not ends %s' failed",  \
                  a, b);                                                       \
  } while (0)
...
static void test_suffix(void) {
  ...
  TEST_SUFFIX_VALID("../files/test-abc.c",
                    "files/{test,value}-{abc,xyz}.{c,h}");
  TEST_SUFFIX_VALID("../files/value-xyz.h",
                    "files/{test,value}-{abc,xyz}.{c,h}");
...
static void test_dir_zero(void) {
  SpDir dir;
...
static void test_dir_single(void) {
  SpDir dir;
  ...
  do {
    ...
    if (rc > 0) {
      ...
      if (dir.pathlen < 9 ||
          memcmp(dir.path + dir.pathlen - 9, ".DS_Store", 9)) {
        n++;
      }
    }
  } while (rc > 0);
...
static void test_dir_tree(void) {
  SpDir dir;
  ...
  do {
    ...
    if (rc > 0) {
      mu_assert_uint_eq(strlen(dir.path), dir.pathlen);
      if (dir.pathlen >= 9 &&
          memcmp(dir.path + dir.pathlen - 9, ".DS_Store", 9) == 0) {
        continue;
      }
