...
static void print_string(FILE *out, const void *val, size_t len) {
...
static void print_value(SpJson *p) {
...
static bool parse(SpJson *p, Message *msg, const uint8_t *in, size_t inlen,
                  ssize_t speed) {
...
static void test_parse(ssize_t speed) {
  SpJson p;
  sp_json_init(&p);
...
static ssize_t parse_value(const uint8_t *m, ssize_t len, uint16_t depth) {
  SpJson p;
  sp_json_init(&p);
...
static ssize_t parse_file(const char *name, uint16_t depth) {
