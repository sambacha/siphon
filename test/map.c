 ...
 */
static uint64_t junk_hash(const void *restrict key, size_t len,
...
static bool key_equals(const void *restrict val, const void *restrict key,
                       size_t len) {
  return strncmp(val, key, len) == 0;
}

SpType junk_type = {
    .hash = junk_hash, .iskey = key_equals, .print = sp_print_str};

SpType good_type = {.hash = sp_metrohash64, .iskey = key_equals};

#define TEST_ADD_NEW(map, key, count)                                          \
  do {                                                                         \
...
#define TEST_ADD_OLD(map, key, count)                                          \
  do {                                                                         \
    int rc = sp_map_put(map, key, strlen(key), key);                           \
...
#define TEST_REM_NEW(map, key, count)                                          \
  do {                                                                         \
...
#define TEST_REM_OLD(map, key, count)                                          \
  do {                                                                         \
    bool rc = sp_map_del(map, key, strlen(key));                               \
...
static void test_single_collision(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
...
static void test_single_bucket_collision(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
...
static void test_single_collision_wrap(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
...
static void test_bucket_collision_steal(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
...
static void test_hash_to_bucket_collision(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
...
static void test_hash_to_bucket_collision2(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
  map.loadf = 0.95;
...
static void test_lookup_collision(void) {
  SpMap map = SP_MAP_MAKE(&junk_type);
...
static void test_downsize(void) {
...
static void *test_copy(void *val) {
  copy_count++;
  return strdup(val);
}

static void test_free(void *val) {
  free_count++;
  free(val);
}

static void test_copy_free(void) {
  SpType type = good_type;
  type.copy = test_copy;
  type.free = test_free;
