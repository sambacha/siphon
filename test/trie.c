...
static const SpType type = {.print = sp_print_str};
...
static void test_put3(void) {
  ...
  mu_assert_str_eq(sp_trie_get(&trie, "abcdefghijklmnopqrstuvwxyz1", 27),
                   "first");
  mu_assert_str_eq(sp_trie_get(&trie, "abcdefghijklmnopqrstuvwxyz2", 27),
                   "second");
...
static void test_get_prefix(void) {
  ...
  mu_assert_str_eq(sp_trie_prefix(&trie, "/test/value", 11, &off, '/'),
                   "first");
  mu_assert_uint_eq(off, 5);

  mu_assert_str_eq(sp_trie_prefix(&trie, "/test/other", 11, &off, '/'),
                   "second");
  mu_assert_uint_eq(off, 11);

  mu_assert_str_eq(sp_trie_prefix(&trie, "/test/other/stuff", 17, &off, '/'),
                   "second");
  mu_assert_uint_eq(off, 11);
...
static void test_get_prefix_no_root(void) {
  SpTrie trie = SP_TRIE_MAKE(&type);
...
static void test_del6(void) {
  SpTrie trie = SP_TRIE_MAKE(&type);

  sp_trie_put(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaa", 25, "first");
  sp_trie_put(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbb", 50,
              "second");
  sp_trie_put(&trie,
              "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbbcccccccccccccc"
              "ccccccccccc",
              75, "third");
  ...
  mu_assert_str_eq(
      sp_trie_get(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbb",
                  50),
      "second");
  mu_assert_str_eq(sp_trie_get(&trie,
                               "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbb"
                               "bbbccccccccccccccccccccccccc",
                               75),
                   "third");
  mu_assert_uint_eq(sp_trie_count(&trie), 2);

  mu_assert_int_eq(
      sp_trie_del(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbb",
                  50),
      true);
  mu_assert_ptr_eq(
      sp_trie_get(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbb",
  ...
  mu_assert_int_eq(sp_trie_del(&trie,
                               "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbb"
                               "bbbccccccccccccccccccccccccc",
                               75),
                   true);
...
static void test_del7(void) {
  ...
  mu_assert_int_eq(sp_trie_del(&trie,
                               ...
                               75),
                   true);
  mu_assert_ptr_eq(sp_trie_get(&trie,
                               "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbb"
                               "bbbccccccccccccccccccccccccc",
                               75),
                   NULL);
  mu_assert_str_eq(
      sp_trie_get(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaabbbbbbbbbbbbbbbbbbbbbbbbb",
                  50),
      "second");
  mu_assert_str_eq(sp_trie_get(&trie, "aaaaaaaaaaaaaaaaaaaaaaaaa", 25),
                   "third");
  mu_assert_uint_eq(sp_trie_count(&trie), 2);
...
static void test_destroy_callback(void *val) {
  Item *item = val;
  item->count++;
}
...
static void test_large_callback(void *val) {
  large_count++;
  free(val);
}
...
static bool test_each_callback(const void *key, size_t len, void *val,
                               void *data) {
  (void)key;
  (void)len;
  (void)val;
...
static bool test_each_prefix_callback(const void *key, size_t len, void *val,
                                      void *data) {
  (void)key;
  (void)len;
  bool *flags = data;
...
static void test_each_prefix(void) {
  SpTrie trie = SP_TRIE_MAKE(&type);

  const char *keys[] = {"aardvark", "dog",     "doge",
                        "dogfish",  "doglike", "zebra"};
...
static void test_each_prefix_leaf(void) {
  SpTrie trie = SP_TRIE_MAKE(&type);

  const char *keys[] = {"aardvark", "dog", "zebra"};
...
#define TEST_PREFIX(t, key, expect, remain)                                    \
  do {                                                                         \
    size_t off = 0;                                                            \
...
static bool matcher(const void *key, size_t len, size_t off, void *val,
                    void *data) {
  (void)val;
...
#define TEST_MATCH(t, key, expect, remain)                                     \
  do {                                                                         \
    MatchData data = {key};                                                    \
