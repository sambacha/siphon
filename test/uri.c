...
#define TEST_PARSE(u, sc, us, pw, hn, pt, pa, q, f)                            \
  do {                                                                         \
    SpUri parsed;                                                              \
    assert_uri(&parsed, u);                                                    \
...
#define TEST_JOIN_SEGMENTS(a, b, sc, us, pw, hn, pt, pa, q, f)                 \
  do {                                                                         \
    char buf[4096];                                                            \
    SpUri ua, ub, join;                                                        \
    assert_uri(&ua, a);                                                        \
...
#define TEST_IPv4_MAP_VALID(str)                                               \
  do {                                                                         \
    SpUri uri;                                                                 \
    const char full[] = "http://[" str "]:1234/path";                          \
    assert_uri(&uri, full);                                                    \
...
#define TEST_IPv4_MAP_INVALID(str)                                             \
  do {                                                                         \
    SpUri uri;                                                                 \
    const char full[] = "http://[" str "]:1234/path";                          \
    ssize_t rc = sp_uri_parse(&uri, full, sizeof full - 1);                    \
...
static void test_parse(void) {
  TEST_PARSE("http://user:pass@www.test.com:80/test/"
             "file.txt?a=b&c=def&x%26y=%C2%AE#frag",
             "http", "user", "pass", "www.test.com", "80", "/test/file.txt",
             "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE(
      "//user:pass@www.test.com:80/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag",
      NULL, "user", "pass", "www.test.com", "80", "/test/file.txt",
      "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE("//www.test.com:80/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag",
             NULL, NULL, NULL, "www.test.com", "80", "/test/file.txt",
             "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE("//www.test.com/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag", NULL,
             NULL, NULL, "www.test.com", NULL, "/test/file.txt",
             "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE("/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag", NULL, NULL, NULL,
             NULL, NULL, "/test/file.txt", "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE("../file.txt?a=b&c=def&x%26y=%C2%AE#frag", NULL, NULL, NULL, NULL,
             NULL, "../file.txt", "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE("?a=b&c=def&x%26y=%C2%AE#frag", NULL, NULL, NULL, NULL, NULL, "",
             "a=b&c=def&x%26y=%C2%AE", "frag");
  TEST_PARSE("#frag", NULL, NULL, NULL, NULL, NULL, "", NULL, "frag");
  TEST_PARSE("http://host:8080/http://www.site.com/images/Mapy%20Region%C3%B3w/"
             "Polska.png?a=b",
             "http", NULL, NULL, "host", "8080",
             "/http://www.site.com/images/Mapy%20Region%C3%B3w/Polska.png",
             "a=b", NULL);
  TEST_PARSE("http://fonts.googleapis.com/"
             "css?family=Cabin+Condensed:600|Cabin:500,700,700italic,500italic",
             "http", NULL, NULL, "fonts.googleapis.com", NULL, "/css",
             "family=Cabin+Condensed:600|Cabin:500,700,700italic,500italic",
             NULL);
  TEST_PARSE("//host.com?a=b", NULL, NULL, NULL, "host.com", NULL, "", "a=b",
             NULL);
}

static void test_sub(void) {
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_FRAGMENT, "http://user:pass@host.com:80/some/path?a=1#frag");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_QUERY, "http://user:pass@host.com:80/some/path?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_PATH, "http://user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_PORT, "http://user:pass@host.com:80");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_HOST, "http://user:pass@host.com");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_PASSWORD, "http://user:pass");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_USER, "http://user");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_SCHEME, "http:");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_USER,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PASSWORD,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_HOST,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PORT,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
           SP_URI_PATH, "/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
           SP_URI_QUERY, "/some/path?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
           SP_URI_FRAGMENT, "/some/path?a=1#frag");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
           SP_URI_QUERY, "?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
           SP_URI_FRAGMENT, "?a=1#frag");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_FRAGMENT,
           SP_URI_FRAGMENT, "#frag");
}

static void test_sub_no_fragment(void) {
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
           SP_URI_FRAGMENT, "http://user:pass@host.com:80/some/path?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
           SP_URI_QUERY, "http://user:pass@host.com:80/some/path?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
           SP_URI_PATH, "http://user:pass@host.com:80/some/path");
  ...
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
           SP_URI_SCHEME, "http:");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_USER,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_PASSWORD,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_HOST,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_PORT,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_PATH,
           SP_URI_PATH, "/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_PATH,
           SP_URI_QUERY, "/some/path?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_PATH,
           SP_URI_FRAGMENT, "/some/path?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_QUERY,
           SP_URI_QUERY, "?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_QUERY,
           SP_URI_FRAGMENT, "?a=1");
  TEST_SUB("http://user:pass@host.com:80/some/path?a=1", SP_URI_FRAGMENT,
           SP_URI_FRAGMENT, "");
}

static void test_sub_no_query(void) {
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
           SP_URI_FRAGMENT, "http://user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
           SP_URI_QUERY, "http://user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME, SP_URI_PATH,
           "http://user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME, SP_URI_PORT,
           "http://user:pass@host.com:80");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME, SP_URI_HOST,
           "http://user:pass@host.com");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
           SP_URI_PASSWORD, "http://user:pass");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME, SP_URI_USER,
           "http://user");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
           SP_URI_SCHEME, "http:");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_USER, SP_URI_PATH,
           "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_PASSWORD,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_HOST, SP_URI_PATH,
           "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_PORT, SP_URI_PATH,
           "//user:pass@host.com:80/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_PATH, SP_URI_PATH,
           "/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_PATH, SP_URI_QUERY,
           "/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_PATH,
           SP_URI_FRAGMENT, "/some/path");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_QUERY, SP_URI_QUERY,
           "");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_QUERY,
           SP_URI_FRAGMENT, "");
  TEST_SUB("http://user:pass@host.com:80/some/path", SP_URI_FRAGMENT,
           SP_URI_FRAGMENT, "");
}

static void test_sub_scheme_rel(void) {
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
           SP_URI_FRAGMENT, "//user:pass@host.com:80/some/path?a=1#frag");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_USER,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PASSWORD,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_HOST,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PORT,
           SP_URI_PATH, "//user:pass@host.com:80/some/path");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
           SP_URI_PATH, "/some/path");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
           SP_URI_QUERY, "/some/path?a=1");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
           SP_URI_FRAGMENT, "/some/path?a=1#frag");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
           SP_URI_QUERY, "?a=1");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
           SP_URI_FRAGMENT, "?a=1#frag");
  TEST_SUB("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_FRAGMENT,
           SP_URI_FRAGMENT, "#frag");
}

static void test_sub_path_rel(void) {
  TEST_SUB("/some/path?a=1#frag", SP_URI_SCHEME, SP_URI_FRAGMENT,
           "/some/path?a=1#frag");
  TEST_SUB("/some/path?a=1#frag", SP_URI_USER, SP_URI_PATH, "/some/path");
  TEST_SUB("/some/path?a=1#frag", SP_URI_PASSWORD, SP_URI_PATH, "/some/path");
  TEST_SUB("/some/path?a=1#frag", SP_URI_HOST, SP_URI_PATH, "/some/path");
  TEST_SUB("/some/path?a=1#frag", SP_URI_PORT, SP_URI_PATH, "/some/path");
  TEST_SUB("/some/path?a=1#frag", SP_URI_PATH, SP_URI_PATH, "/some/path");
  TEST_SUB("/some/path?a=1#frag", SP_URI_PATH, SP_URI_QUERY, "/some/path?a=1");
  TEST_SUB("/some/path?a=1#frag", SP_URI_PATH, SP_URI_FRAGMENT,
           "/some/path?a=1#frag");
  TEST_SUB("/some/path?a=1#frag", SP_URI_QUERY, SP_URI_QUERY, "?a=1");
  TEST_SUB("/some/path?a=1#frag", SP_URI_QUERY, SP_URI_FRAGMENT, "?a=1#frag");
  TEST_SUB("/some/path?a=1#frag", SP_URI_FRAGMENT, SP_URI_FRAGMENT, "#frag");
}

static void test_sub_query_rel(void) {
  TEST_SUB("?a=1#frag", SP_URI_SCHEME, SP_URI_FRAGMENT, "?a=1#frag");
  TEST_SUB("?a=1#frag", SP_URI_USER, SP_URI_PATH, "");
  TEST_SUB("?a=1#frag", SP_URI_PASSWORD, SP_URI_PATH, "");
  TEST_SUB("?a=1#frag", SP_URI_HOST, SP_URI_PATH, "");
  TEST_SUB("?a=1#frag", SP_URI_PORT, SP_URI_PATH, "");
  TEST_SUB("?a=1#frag", SP_URI_PATH, SP_URI_PATH, "");
  TEST_SUB("?a=1#frag", SP_URI_PATH, SP_URI_QUERY, "?a=1");
  TEST_SUB("?a=1#frag", SP_URI_PATH, SP_URI_FRAGMENT, "?a=1#frag");
  TEST_SUB("?a=1#frag", SP_URI_QUERY, SP_URI_QUERY, "?a=1");
  TEST_SUB("?a=1#frag", SP_URI_QUERY, SP_URI_FRAGMENT, "?a=1#frag");
  TEST_SUB("?a=1#frag", SP_URI_FRAGMENT, SP_URI_FRAGMENT, "#frag");
}

static void test_sub_fragment_rel(void) {
  TEST_SUB("#frag", SP_URI_SCHEME, SP_URI_FRAGMENT, "#frag");
  TEST_SUB("#frag", SP_URI_USER, SP_URI_PATH, "");
  TEST_SUB("#frag", SP_URI_PASSWORD, SP_URI_PATH, "");
  TEST_SUB("#frag", SP_URI_HOST, SP_URI_PATH, "");
  TEST_SUB("#frag", SP_URI_PORT, SP_URI_PATH, "");
  TEST_SUB("#frag", SP_URI_PATH, SP_URI_PATH, "");
  TEST_SUB("#frag", SP_URI_PATH, SP_URI_QUERY, "");
  TEST_SUB("#frag", SP_URI_PATH, SP_URI_FRAGMENT, "#frag");
  TEST_SUB("#frag", SP_URI_QUERY, SP_URI_QUERY, "");
  TEST_SUB("#frag", SP_URI_QUERY, SP_URI_FRAGMENT, "#frag");
  TEST_SUB("#frag", SP_URI_FRAGMENT, SP_URI_FRAGMENT, "#frag");
}

static void test_range(void) {
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
             SP_URI_FRAGMENT,
  ...
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
             SP_URI_SCHEME, "http");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_USER,
             SP_URI_PATH, "user:pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PASSWORD,
             SP_URI_PATH, "pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_HOST,
             SP_URI_PATH, "host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PORT,
             SP_URI_PATH, "80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
             SP_URI_PATH, "/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
             SP_URI_QUERY, "/some/path?a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
             SP_URI_FRAGMENT, "/some/path?a=1#frag");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
             SP_URI_QUERY, "a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
             SP_URI_FRAGMENT, "a=1#frag");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_FRAGMENT,
             SP_URI_FRAGMENT, "frag");
}

static void test_range_no_fragment(void) {
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
             SP_URI_FRAGMENT, "http://user:pass@host.com:80/some/path?a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
             SP_URI_QUERY, "http://user:pass@host.com:80/some/path?a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
  ...
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_SCHEME,
             SP_URI_SCHEME, "http");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_USER,
             SP_URI_PATH, "user:pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_PASSWORD,
             SP_URI_PATH, "pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_HOST,
             SP_URI_PATH, "host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_PORT,
             SP_URI_PATH, "80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_PATH,
             SP_URI_PATH, "/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_PATH,
             SP_URI_QUERY, "/some/path?a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_PATH,
             SP_URI_FRAGMENT, "/some/path?a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_QUERY,
             SP_URI_QUERY, "a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1", SP_URI_QUERY,
             SP_URI_FRAGMENT, "a=1");
  TEST_RANGE("http://user:pass@host.com:80/some/path?a=1#frag", SP_URI_FRAGMENT,
...
static void test_range_no_query(void) {
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
             SP_URI_FRAGMENT, "http://user:pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
             SP_URI_QUERY, "http://user:pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
  ...
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_SCHEME,
             SP_URI_SCHEME, "http");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_USER, SP_URI_PATH,
             "user:pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_PASSWORD,
             SP_URI_PATH, "pass@host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_HOST, SP_URI_PATH,
             "host.com:80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_PORT, SP_URI_PATH,
             "80/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_PATH, SP_URI_PATH,
             "/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_PATH,
             SP_URI_QUERY, "/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_PATH,
             SP_URI_FRAGMENT, "/some/path");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_QUERY,
             SP_URI_QUERY, "");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_QUERY,
             SP_URI_FRAGMENT, "");
  TEST_RANGE("http://user:pass@host.com:80/some/path", SP_URI_FRAGMENT,
             SP_URI_FRAGMENT, "");
}

static void test_range_scheme_rel(void) {
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_SCHEME,
             SP_URI_FRAGMENT, "user:pass@host.com:80/some/path?a=1#frag");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_USER,
             SP_URI_PATH, "user:pass@host.com:80/some/path");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PASSWORD,
             SP_URI_PATH, "pass@host.com:80/some/path");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_HOST,
             SP_URI_PATH, "host.com:80/some/path");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PORT,
             SP_URI_PATH, "80/some/path");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
             SP_URI_PATH, "/some/path");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
             SP_URI_QUERY, "/some/path?a=1");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_PATH,
             SP_URI_FRAGMENT, "/some/path?a=1#frag");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
             SP_URI_QUERY, "a=1");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_QUERY,
             SP_URI_FRAGMENT, "a=1#frag");
  TEST_RANGE("//user:pass@host.com:80/some/path?a=1#frag", SP_URI_FRAGMENT,
             SP_URI_FRAGMENT, "frag");
}

static void test_range_path_rel(void) {
  TEST_RANGE("/some/path?a=1#frag", SP_URI_SCHEME, SP_URI_FRAGMENT,
             "/some/path?a=1#frag");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_USER, SP_URI_PATH, "/some/path");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_PASSWORD, SP_URI_PATH, "/some/path");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_HOST, SP_URI_PATH, "/some/path");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_PORT, SP_URI_PATH, "/some/path");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_PATH, SP_URI_PATH, "/some/path");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_PATH, SP_URI_QUERY,
             "/some/path?a=1");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_PATH, SP_URI_FRAGMENT,
             "/some/path?a=1#frag");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_QUERY, SP_URI_QUERY, "a=1");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_QUERY, SP_URI_FRAGMENT, "a=1#frag");
  TEST_RANGE("/some/path?a=1#frag", SP_URI_FRAGMENT, SP_URI_FRAGMENT, "frag");
}

static void test_range_query_rel(void) {
  TEST_RANGE("?a=1#frag", SP_URI_SCHEME, SP_URI_FRAGMENT, "a=1#frag");
  TEST_RANGE("?a=1#frag", SP_URI_USER, SP_URI_PATH, "");
  TEST_RANGE("?a=1#frag", SP_URI_PASSWORD, SP_URI_PATH, "");
  TEST_RANGE("?a=1#frag", SP_URI_HOST, SP_URI_PATH, "");
  TEST_RANGE("?a=1#frag", SP_URI_PORT, SP_URI_PATH, "");
  TEST_RANGE("?a=1#frag", SP_URI_PATH, SP_URI_PATH, "");
  TEST_RANGE("?a=1#frag", SP_URI_PATH, SP_URI_QUERY, "a=1");
  TEST_RANGE("?a=1#frag", SP_URI_PATH, SP_URI_FRAGMENT, "a=1#frag");
  TEST_RANGE("?a=1#frag", SP_URI_QUERY, SP_URI_QUERY, "a=1");
  TEST_RANGE("?a=1#frag", SP_URI_QUERY, SP_URI_FRAGMENT, "a=1#frag");
  TEST_RANGE("?a=1#frag", SP_URI_FRAGMENT, SP_URI_FRAGMENT, "frag");
}

static void test_range_fragment_rel(void) {
  TEST_RANGE("#frag", SP_URI_SCHEME, SP_URI_FRAGMENT, "frag");
  TEST_RANGE("#frag", SP_URI_USER, SP_URI_PATH, "");
  TEST_RANGE("#frag", SP_URI_PASSWORD, SP_URI_PATH, "");
  TEST_RANGE("#frag", SP_URI_HOST, SP_URI_PATH, "");
  TEST_RANGE("#frag", SP_URI_PORT, SP_URI_PATH, "");
  TEST_RANGE("#frag", SP_URI_PATH, SP_URI_PATH, "");
  TEST_RANGE("#frag", SP_URI_PATH, SP_URI_QUERY, "");
  TEST_RANGE("#frag", SP_URI_PATH, SP_URI_FRAGMENT, "frag");
  TEST_RANGE("#frag", SP_URI_QUERY, SP_URI_QUERY, "");
  TEST_RANGE("#frag", SP_URI_QUERY, SP_URI_FRAGMENT, "frag");
  TEST_RANGE("#frag", SP_URI_FRAGMENT, SP_URI_FRAGMENT, "frag");
}
...
static void test_join(void) {
  ...
  TEST_JOIN("http://user:pass@test.com:80/some/path?a=b#c",
            "//new.com/new/path", "http://user:pass@new.com/new/path");
  ...
  TEST_JOIN_SEGMENTS(
      "http://base.com/some/path.html?a=b#c:w",
      "//www.test.com:80/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag", "http",
      NULL, NULL, "www.test.com", "80", "/test/file.txt",
      "a=b&c=def&x%26y=%C2%AE", "frag");

  TEST_JOIN_SEGMENTS("http://base.com/some/path.html?a=b#c:w",
                     "//www.test.com/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag",
                     "http", NULL, NULL, "www.test.com", NULL, "/test/file.txt",
                     "a=b&c=def&x%26y=%C2%AE", "frag");

  TEST_JOIN_SEGMENTS("//www.test.com/some/path.html", "../other.html", NULL,
                     NULL, NULL, "www.test.com", NULL, "/other.html", NULL,
                     NULL);

  TEST_JOIN_SEGMENTS("http://base.com/some/path.html?a=b#c:w",
                     "/test/file.txt?a=b&c=def&x%26y=%C2%AE#frag", "http", NULL,
                     NULL, "base.com", NULL, "/test/file.txt",
                     "a=b&c=def&x%26y=%C2%AE", "frag");

  TEST_JOIN_SEGMENTS("/some/path.html", "../other.html", NULL, NULL, NULL, NULL,
                     NULL, "/other.html", NULL, NULL);

  TEST_JOIN_SEGMENTS("http://base.com/some/path.html?a=b#c:w",
                     "../file.txt?a=b&c=def&x%26y=%C2%AE#frag", "http", NULL,
                     NULL, "base.com", NULL, "/file.txt",
                     "a=b&c=def&x%26y=%C2%AE", "frag");

  TEST_JOIN_SEGMENTS("http://base.com/some/path.html?a=b#c:w",
                     "?a=b&c=def&x%26y=%C2%AE#frag", "http", NULL, NULL,
                     "base.com", NULL, "/some/path.html",
                     "a=b&c=def&x%26y=%C2%AE", "frag");

  TEST_JOIN_SEGMENTS("/some/path.html", "?a=b", NULL, NULL, NULL, NULL, NULL,
                     "/some/path.html", "a=b", NULL);

  TEST_JOIN_SEGMENTS("http://base.com/some/path.html?a=b#c:w", "#frag", "http",
                     NULL, NULL, "base.com", NULL, "/some/path.html", "a=b",
                     "frag");

  TEST_JOIN_SEGMENTS("http://www.test.com/", "http://www.test.com/dir/", "http",
                     NULL, NULL, "www.test.com", NULL, "/dir/", NULL, NULL);

  TEST_JOIN_SEGMENTS("http://www.test.com/dir/", "file.html", "http", NULL,
                     NULL, "www.test.com", NULL, "/dir/file.html", NULL, NULL);

  TEST_JOIN_SEGMENTS("http://user@test.com:80/some/path?a=b#c",
                     "//new.com/new/path", "http", "user", NULL, "new.com",
                     NULL, "/new/path", NULL, NULL);
}
...
static void test_ipv6(void) {
  ...
  TEST_IPv6_VALID("::0:0:0:0:0:0");
  TEST_IPv6_VALID("::0:0:0:0:0:0:0");
  TEST_IPv6_VALID("::0:a:b:c:d:e:f"); // syntactically correct, but bad form
                                      // (::0:... could be combined)
  TEST_IPv6_VALID("::1");
  TEST_IPv6_VALID("::1");
  TEST_IPv6_VALID("::1");
  ...
  TEST_IPv4_MAP_VALID("0:0:0:0:0:0:13.1.68.3"); // IPv4-compatible IPv6 address,
                                                // full, deprecated
  TEST_IPv4_MAP_VALID(
...
static void test_query_single_key(void) {
...
static void test_query_single_value(void) {
...
static void test_query_utf8(void) {
  const char input[] =
      "http://test.com/some/path?x=123&text=drop+the+%22bass%22%0A%F0%9D%84%A2";
  SpUri uri;

  ssize_t rc = sp_uri_parse(&uri, input, sizeof input - 1);
  mu_fassert_msg(rc >= 0, "Failed to parse URI \"%s\"", input);

  const char *cmp[][2] = {{"x", "123"},
                          {"text", "drop the \"bass\"\n\xF0\x9D\x84\xA2"}};
...
static void test_query_utf8_bad(void) {
  const char input[] =
      "http://test.com/some/path?x=123&text=drop+the+%22bass%22%0A%F0%9D%84%22";
  SpUri uri;
...
static void test_query_mess(void) {
  const char input[] =
      "http://test.com/some/path?&&&&x&&&a=1&&&&test=stuff&y=1=2&z&&&&&";
  SpUri uri;
