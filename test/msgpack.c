...
static ssize_t parse(Message *msg, const uint8_t *in, size_t inlen,
                     ssize_t speed) {
#if DEBUG
  printf("****************************************************************\n");
#endif
...
static void test_decode_general(ssize_t speed) {
  /*
   * ["test",
  ...
  static const uint8_t input[] = {
      };
  Message msg;
...
static void test_decode_nested_begin(ssize_t speed) {
  /*
   * {"test"=>["a", "b", "c"], "stuff"=>"123", "other"=>"456"}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0x93, 0xa1, 0x61, 0xa1, 0x62,
      0xa1, 0x63, 0xa5, 0x73, 0x74, 0x75, 0x66, 0x66, 0xa3, 0x31, 0x32,
      0x33, 0xa5, 0x6f, 0x74, 0x68, 0x65, 0x72, 0xa3, 0x34, 0x35, 0x36};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
...
static void test_decode_nested_middle(ssize_t speed) {
  /*
   * {"test"=>"123", "stuff"=>["a", "b", "c"], "other"=>"456"}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0xa3, 0x31, 0x32, 0x33, 0xa5,
      0x73, 0x74, 0x75, 0x66, 0x66, 0x93, 0xa1, 0x61, 0xa1, 0x62, 0xa1,
      0x63, 0xa5, 0x6f, 0x74, 0x68, 0x65, 0x72, 0xa3, 0x34, 0x35, 0x36};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
  mu_fassert_uint_eq(msg.field_count, 12);
  mu_assert_int_eq(msg.fields[0].type, SP_MSGPACK_MAP);
...
static void test_decode_nested_end(ssize_t speed) {
  /*
   * {"test"=>"123", "stuff"=>"456", "other"=>["a", "b", "c"]}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0xa3, 0x31, 0x32, 0x33, 0xa5,
      0x73, 0x74, 0x75, 0x66, 0x66, 0xa3, 0x34, 0x35, 0x36, 0xa5, 0x6f,
      0x74, 0x68, 0x65, 0x72, 0x93, 0xa1, 0x61, 0xa1, 0x62, 0xa1, 0x63};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
  mu_fassert_uint_eq(msg.field_count, 12);
  mu_assert_int_eq(msg.fields[0].type, SP_MSGPACK_MAP);
...
static void test_decode_array_key_begin(ssize_t speed) {
  /*
   * {["x", "y", "z"]=>["a", "b", "c"], "stuff"=>"123", "other"=>"456"}
   */
  static const uint8_t input[] = {
      0x83, 0x93, 0xa1, 0x78, 0xa1, 0x79, 0xa1, 0x7a, 0x93, 0xa1, 0x61, 0xa1,
      0x62, 0xa1, 0x63, 0xa5, 0x73, 0x74, 0x75, 0x66, 0x66, 0xa3, 0x31, 0x32,
      0x33, 0xa5, 0x6f, 0x74, 0x68, 0x65, 0x72, 0xa3, 0x34, 0x35, 0x36};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
...
static void test_decode_array_key_middle(ssize_t speed) {
  /*
   * {"test"=>"123", ["x", "y", "z"]=>["a", "b", "c"], "other"=>"456"}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0xa3, 0x31, 0x32, 0x33, 0x93, 0xa1,
      0x78, 0xa1, 0x79, 0xa1, 0x7a, 0x93, 0xa1, 0x61, 0xa1, 0x62, 0xa1, 0x63,
      0xa5, 0x6f, 0x74, 0x68, 0x65, 0x72, 0xa3, 0x34, 0x35, 0x36};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
  mu_fassert_uint_eq(msg.field_count, 16);
  mu_assert_int_eq(msg.fields[0].type, SP_MSGPACK_MAP);
...
static void test_decode_array_key_end(ssize_t speed) {
  /*
   * {"test"=>"123", "stuff"=>"456", ["x", "y", "z"]=>["a", "b", "c"]}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0xa3, 0x31, 0x32, 0x33, 0xa5, 0x73,
      0x74, 0x75, 0x66, 0x66, 0xa3, 0x34, 0x35, 0x36, 0x93, 0xa1, 0x78, 0xa1,
      0x79, 0xa1, 0x7a, 0x93, 0xa1, 0x61, 0xa1, 0x62, 0xa1, 0x63};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
  mu_fassert_uint_eq(msg.field_count, 16);
  mu_assert_int_eq(msg.fields[0].type, SP_MSGPACK_MAP);
...
static void test_decode_map_key_begin(ssize_t speed) {
  /*
   * {{"x"=>"y"}=>["a", "b", "c"], "stuff"=>"123", "other"=>"456"}
   */
  static const uint8_t input[] = {
      0x83, 0x81, 0xa1, 0x78, 0xa1, 0x79, 0x93, 0xa1, 0x61, 0xa1, 0x62,
      0xa1, 0x63, 0xa5, 0x73, 0x74, 0x75, 0x66, 0x66, 0xa3, 0x31, 0x32,
      0x33, 0xa5, 0x6f, 0x74, 0x68, 0x65, 0x72, 0xa3, 0x34, 0x35, 0x36};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
...
static void test_decode_map_key_middle(ssize_t speed) {
  /*
   * {"test"=>"123", {"x"=>"y"}=>["a", "b", "c"], "other"=>"456"}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0xa3, 0x31, 0x32, 0x33, 0x81,
      0xa1, 0x78, 0xa1, 0x79, 0x93, 0xa1, 0x61, 0xa1, 0x62, 0xa1, 0x63,
      0xa5, 0x6f, 0x74, 0x68, 0x65, 0x72, 0xa3, 0x34, 0x35, 0x36};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
  mu_fassert_uint_eq(msg.field_count, 15);
...
static void test_decode_map_key_end(ssize_t speed) {
  /*
   * {"test"=>"123", "stuff"=>"456", {"x"=>"y"}=>["a", "b", "c"]}
   */
  static const uint8_t input[] = {
      0x83, 0xa4, 0x74, 0x65, 0x73, 0x74, 0xa3, 0x31, 0x32, 0x33, 0xa5,
      0x73, 0x74, 0x75, 0x66, 0x66, 0xa3, 0x34, 0x35, 0x36, 0x81, 0xa1,
      0x78, 0xa1, 0x79, 0x93, 0xa1, 0x61, 0xa1, 0x62, 0xa1, 0x63};
  Message msg;
  mu_fassert_uint_eq(parse(&msg, input, sizeof input, speed), sizeof input);
  mu_fassert_uint_eq(msg.field_count, 15);
  mu_assert_int_eq(msg.fields[0].type, SP_MSGPACK_MAP);
...
static void test_decode(ssize_t speed) {
...
static void test_encode_nil(void) {
  uint8_t buf[64];
  ssize_t rc;
...
static void test_encode_true(void) {
  uint8_t buf[64];
  ssize_t rc;
...
static void test_encode_false(void) {
  uint8_t buf[64];
  ssize_t rc;
...
static void test_encode_signed(void) {
  uint8_t buf[64], *m;
  ssize_t rc;
...
static void test_encode_unsigned(void) {
  uint8_t buf[64], *m;
  ssize_t rc;
...
static void test_encode_float(void) {
  uint8_t buf[64], *m;
  ssize_t rc;
...
static void test_encode_string() {
  ...
  const char *values[] = {
      "test", "this is a longer string that should not pack to a fixed string"};
